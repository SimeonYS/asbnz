...
class AsbnzPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('asbnz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `asbnz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM asbnz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO asbnz (date, title, link, content)"
